...
$BuildDir = Join-Path $BackendDir "build-release"
$DepsDir = Join-Path $BackendDir ".deps-release"

if ($Clean) {
    Write-Host "Cleaning release build and deps directory..."
    if (Test-Path $BuildDir) { Remove-Item -Recurse -Force $BuildDir }
    if (Test-Path $DepsDir) { Remove-Item -Recurse -Force $DepsDir }
}

if ($Test) {
    Write-Host "Cleaning release test artifacts..."
...
Write-Host "Building QuickMemes Backend (Release)..."
...
& cmake --build $BuildDir --parallel $cpuCount --config Release
