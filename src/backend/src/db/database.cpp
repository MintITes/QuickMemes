...
bool Database::initialize(const std::string &dbPath) {
	...
	try {
		...
		if (dbPath == ":memory:") {
			flags = SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE | SQLite::OPEN_URI;
		}
		...
		// 注册 regexp 函数
		sqlite3_create_function(db_->getHandle(), "regexp", 2, SQLITE_UTF8 | SQLITE_DETERMINISTIC, nullptr, regexp_func,
		                        nullptr, nullptr);
...
MemeEntry Database::getMeme(int64_t id) {
	try {
		...
		if (!stmt.getColumn(7).isNull())
			meme.sourceName = stmt.getColumn(7).getString();
		if (!stmt.getColumn(8).isNull())
			meme.sourceUrl = stmt.getColumn(8).getString();
		if (!stmt.getColumn(9).isNull())
			meme.name = stmt.getColumn(9).getString();
		if (!stmt.getColumn(10).isNull())
			meme.description = stmt.getColumn(10).getString();
		if (!stmt.getColumn(11).isNull())
			meme.ocrText = stmt.getColumn(11).getString();
...
PagedMemeResults Database::searchMemes(const SearchQuery &query) {
	...
	try {
		...
		if (!query.keyword.empty()) {
			sql += " JOIN memes_fts mf ON m.id = mf.rowid ";
		}
...
std::vector<MemeEntry> Database::vectorSearch(const std::vector<float> &embedding, int limit) {
	std::shared_lock lock(dbMutex_);
	if (embedding.empty()) {
		throw ApiException(ERR_INVALID_PARAMS, "Empty embedding provided for vector search");
	}
...
bool Database::updateMemeProcessing(int64_t id, ProcessingStatus ocrStatus, ProcessingStatus aiStatus,
                                    const std::string &ocrText, const std::string &description) {
	try {
		SQLite::Statement stmt(*db_, "UPDATE memes SET updated_at = ?, ocr_status = ?, ai_status = ?, ocr_text = ?, "
...
std::vector<Tag> Database::getTags() {
	try {
		...
		std::vector<Tag> tags;
		while (stmt.executeStep()) {
			tags.push_back({stmt.getColumn(0).getInt64(), stmt.getColumn(1).getString(), stmt.getColumn(2).getString(),
...
int64_t Database::insertCategory(const Category &category) {
	try {
		SQLite::Statement stmt(*db_, "INSERT INTO categories (uuid, name, color, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
		
		auto nowMs = std::chrono::duration_cast<std::chrono::milliseconds>(
		                 std::chrono::system_clock::now().time_since_epoch()).count();
		
		std::string uuid = category.uuid.empty() ? "uuid_placeholder" : category.uuid; // Ideally use a real UUID generator
...
std::vector<Category> Database::getCategories() {
	try {
		SQLite::Statement stmt(*db_, "SELECT id, uuid, name, color, created_at, updated_at FROM categories ORDER BY name ASC");
...
void Database::upsertEmbedding(int64_t memeId, const std::vector<float> &embedding) {
	...
	try {
		...
		LOG_ERROR("persist", std::string("upsertEmbedding failed: ") + e.what());
	}
}
...
void Database::rebuildVecTable(int newDimension) {
	std::unique_lock lock(dbMutex_);
	if (newDimension <= 0) {
		newDimension = VisionModule::get().getEmbeddingDimension();
	}
	if (newDimension <= 0)
		newDimension = 1536; // Fallback
	...
	try {
		...
		LOG_ERROR("persist", std::string("rebuildVecTable failed: ") + e.what());
	}
}
...
void Database::runMigrations() {
	...
	if (currentVersion < 1) {
		...
		if (dim <= 0) dim = 1536; 
		db_->exec("CREATE VIRTUAL TABLE IF NOT EXISTS vec_memes USING vec0(meme_id INTEGER PRIMARY KEY, embedding float[" +
		          std::to_string(dim) + "]);");
...
SearchSql Database::buildSearchSql(const SearchQuery &query) {
	...
	if (query.tagIds.size() > 900) {
		throw ApiException(ERR_INVALID_PARAMS, "Too many tags in query (limit 900)");
	}
