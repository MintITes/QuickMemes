...
TEST(ConfigParserTest, ParseArgs_StringFlagWithoutValue_Throws) {
	const std::vector<const char *> rawArgs = {"quickmemes",
	                                           "--bind-address",
	                                           "--port",
	                                           "8080"};
	...
	EXPECT_THROW(
	    {
		    try {
			    ...
			    EXPECT_NE(std::string(e.what()).find("missing value for --bind-address"), std::string::npos);
			    throw;
		    }
	    },
	    std::invalid_argument);
...
TEST(ConfigParserTest, ParseArgs_InvalidBoolValue_Throws) {
	...
	EXPECT_THROW(
	    {
		    try {
			    ...
			    EXPECT_NE(std::string(e.what()).find("invalid boolean value for --log-retention-enabled"),
			              std::string::npos);
			    throw;
		    }
	    },
...
TEST(ConfigParserTest, ParseArgs_BoolFlagWithoutValue_IsAcceptedAsTrue) {
	const std::vector<const char *> rawArgs = {"quickmemes",
	                                           "--bind-address",
	                                           "0.0.0.0",
	                                           "--port",
	...
	auto config = parseArgs(static_cast<int>(argv.size()), argv.data());
	EXPECT_TRUE(config.logRetentionEnabled);
	EXPECT_TRUE(config.thumbnailEnabled);
}

TEST(ConfigParserTest, ParseArgs_OptionalFields_DefaultsApplied) {
	const std::vector<const char *> rawArgs = {"quickmemes",
	                                           "--bind-address",
	                                           "127.0.0.1",
	                                           "--port",
	...
	auto config = parseArgs(static_cast<int>(argv.size()), argv.data());
	EXPECT_TRUE(config.logRetentionEnabled);
	EXPECT_EQ(config.logRetentionDays, 30);
	EXPECT_EQ(config.visionConfig.timeoutSeconds, 30);
	EXPECT_EQ(config.visionConfig.maxRetries, 2);
	EXPECT_EQ(config.embeddingConfig.dimensions, 512);
	EXPECT_EQ(config.embeddingConfig.timeoutSeconds, 30);
	EXPECT_EQ(config.embeddingConfig.maxRetries, 2);
	EXPECT_TRUE(config.thumbnailEnabled);
	EXPECT_EQ(config.thumbnailMaxSize, 300);
	EXPECT_TRUE(config.backupEnabled);
	EXPECT_EQ(config.backupRetentionDays, 30);
	EXPECT_EQ(config.recycleBinRetentionDays, 30);
	EXPECT_EQ(config.maxQueueSize, 500);
	EXPECT_EQ(config.workerCount, 4);
}
