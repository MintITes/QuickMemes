...
为了平衡操作速度与误触防范，采用分层复制策略：

1.  **直观入口**：MemeCard 悬停时右下角出现的 **📋 图标**，单击立即完成。
2.  **效率快捷**：鼠标**双击**卡片任意区域，触发高亮闪烁反馈并完成复制。
3.  **管理入口**：右键菜单中的“复制到剪贴板”。
4.  **键盘流**：选中卡片（单击）后，按 `Ctrl+C`。

> 动效反馈：所有复制动作触发后，卡片应有一次**明度的快速闪烁**，并在屏幕右下角弹出 Toast。
...
| 鼠标框选 (Rubber Band) | 在空白区域按住左键拖动，画出矩形，框内梗图全部选中。**框选期间，非激活区域产生微弱压暗/模糊效果，增强操作感**。 |
| **Ctrl + 鼠标滚轮**    | **动态缩放梗图尺寸，并联动调整网格间距 (Gap)**                                                                  |

### 多选反馈与浮动操作条 (FAB) [NEW]

当选中项 **≥ 2** 时，系统会同时提供两种操作入口：

1.  **右侧面板 (Inspector)**：展示详细属性的批量编辑（如修改标签名）。
2.  **底部浮动操作条 (FloatActionBar)**：位于主区域底部的水平条，提供最高频的批量操作（复制、导出、存入收集桶、删除）。

```mermaid
flowchart TD
    A[用户选中 2+ 张梗图] --> B[底部平滑弹出 FloatActionBar]
    A --> C[右侧 Inspector 滑出并展示批量编辑界面]
    B --> D{点击 FAB 按钮}
    D -->|复制/导出| E[执行对应批量动作]
    D -->|存入魔法桶| F[ID 集合批量注入状态机]
    D -->|删除| G[移入回收站并显示撤销 Toast]
    A -->|取消所有选择| H[FAB 自动缩回并消失]
```
...
sequenceDiagram
    ...
    用户->>左侧栏: 释放鼠标（Drop）
    left_sidebar->>React: 触发归类操作
    React->>后端: PUT /api/meme/{id} MemePatch { categoryId: {targetId} }
...
| **磁性吸附** | **当鼠标距离有效目标 <40px 时，目标图标产生微弱引力感并伴随张口/放大微动效** |
| 悬停有效目标 | 目标分类项高亮发光边框                                                       |
| 悬停无效目标 | 无特殊视觉提示                                                               |
...
| 方式           | 说明                                                                           |
| :------------- | :----------------------------------------------------------------------------- |
| 主动导出       | 通过右键菜单或 FAB 点击导出，弹出系统另存为对话框                              |
| **跨容器拖拽** | **直接从窗口内将卡片拖向桌面、WeChat、DingTalk 等外部应用，实现即时发送/保存** |
...
sequenceDiagram
    ...
    用户->>主区域: 拖拽一张或多张梗图
    用户->>魔法桶: 悬停在侧边栏"收集桶"上方
    Note over 魔法桶: 图标变为"开启"状态并轻微震动
    用户->>魔法桶: 释放鼠标
    魔法桶->>React状态: addItemsToBucket(ids)
    React状态->>魔法桶: 更新角标计数
    React状态->>用户: 弹出微型 Toast "已暂存 N 张"
```
...
flowchart LR
    A[点击收集桶] --> B[展开暂存悬浮层]
    B --> C{用户选择}
    C -->|一键复制| D[合并导出或顺序写入剪贴板]
    C -->|创建合集| E[弹出“新建分类”弹窗并转移数据]
    C -->|一键清空| F[重置桶状态]
    C -->|移除单项| G[从暂存列表剔除]
```
...
## 发现性增强 (Discoverability) [NEW]

### 全局快捷键引导 (Keymap Overlay)

由于系统支持全键盘深度操作，长按 `Ctrl` 键可触发全局引导层：

1.  **触发**：按下 `Ctrl` 并保持约 1 秒。
2.  **视觉**：界面整体变暗并虚化，屏幕中央通过栅格展示当前视图可用的所有快捷键。
3.  **消失**：松开按键，一切恢复原位，无感知切换。
