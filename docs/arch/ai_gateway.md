...
- 图像多模态分析：根据图像内容建议标签和生成描述文本
- 文本语义向量化：将文本转换为浮点数向量，供 sqlite-vec 进行相似度搜索
- AI 图像生成：根据文字描述调用图像生成 API
...
graph TD
    ...
    CORE -->|"analyzeImage() / generateEmbedding() / generateImage()"| AI_MOD
    HTTP_CLI -.->|"HTTPS REST"| CLOUD
    CLOUD -.->|"JSON 响应"| HTTP_CLI
    AI_MOD -->|"AiAnalysisResult / float[] / GeneratedImage"| CORE
```
